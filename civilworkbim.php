...
$exists=false;
	...
	// if($num == 0) {
		if(($phone != NULL) && $exists==false) {			
			$status = "true";
			$sql = "INSERT INTO `workshop_data` (`civil_id`,`user_name`,`email`, `phone` , `payment_status`, `custom_data`, `timestamp`, `workshop1_bim`, `worshop2_steel`, `worshop3_ultra`) VALUES ('$civil_id','$user_name','$email','$phone','$payment_status',current_timestamp(),current_timestamp(),'$true','$false','$false')";
	
			$result = mysqli_query($conn, $sql);
	
			if ($result) {
				session_start();
				$_SESSION["bim_workshop_status"]=$status;
				$_SESSION["bim_workshop_payment_status"]="false";
				$showAlert = true;
			}
		}
		else {
...
// if($num>0)
// {
// 	$exists="Already Registered for BIM Workshop !";
// }
