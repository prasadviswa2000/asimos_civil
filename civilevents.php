...
$exists=false;
	...
	$sql = "Select * from events_data where phone='$phone'";
	...
	if($num == 0) {
		if(($phone != NULL) && $exists==false) {			
			$status = "true";
			$sql = "INSERT INTO `events_data` (`civil_id`,`user_name`,
				`status`, `email`, `phone`, `payment_status`,`timestamp`) VALUES ('$civil_id','$user_name',
				'$status','$email','$phone','$payment_status',current_timestamp())";
	
			$result = mysqli_query($conn, $sql);
	
			if ($result) {
				session_start();
				$_SESSION["event_status"]=$status;
				$_SESSION["events_payment_status"]="false";
				$showAlert = true;
			}
		}
		else {
...
if($num>0)
{
	$exists="Already Registered for events !";
}
